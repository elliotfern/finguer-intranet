              ...
              if (isset($_POST["add-vol"])) {
               ...
               // Si no hi ha cap error, envia el formulari
                if (!isset($hasError)) {
                    ...
                    if ($codi_resposta == 1)  {
                    echo '<div class="alert alert-success" role="alert"><h4 class="alert-heading"><strong>Vol afegit correctament.</h4></strong>';
                    echo "El vol afegit és: ".$_POST["vuelo"]."</div>";
                    } else { // Error > bloqueja i mostra avis
                ...
                if ($codi_resposta == 2) { 
                    echo '<form action="" method="post" id="add-vol" class="row g-3" style="background-color:#BDBDBD;padding:25px;margin-top:10px">';

                    echo '<input type="hidden" id="id" name="id" value="'.$id_old.'">';
                    ...
                    echo "<button id='add-vol' name='add-vol' type='submit' class='btn btn-primary'>Afegir vol</button><a href='afegir-vol.php'></a>
                    </div>";
