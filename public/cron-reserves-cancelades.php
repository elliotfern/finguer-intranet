...
AND p.post_type='shop_order' AND p.post_status IN ('wc-cancelled') AND NOT r.checkIn = 6 )";
...
while ($i <= $numero) {
    ...
    (SELECT p.ID FROM wpfin_posts as p
     WHERE p.ID = r.idReserva AND p.post_type='shop_order' AND p.post_status IN ('wc-cancelled') AND NOT r.checkIn = 6 ) 
    LIMIT 1";
